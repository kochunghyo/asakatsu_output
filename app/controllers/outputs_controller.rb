...
class OutputsController < ApplicationController
  ...
  def create
    if Time.zone.now.hour < 4 || Time.zone.now.hour >= 10
  ...
  def output_params
    params.require(:output).permit(:title, :text, :type_id,
                                   images_attributes: %i[src _destroy id]).merge(user_id: current_user.id)
  end
